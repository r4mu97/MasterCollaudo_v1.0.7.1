...
Namespace norvanco.http
    ...
    Public Class MultipartForm
        ...
        Public Sub sendFile(ByVal aFilename As String, ByVal cookieContainer As CookieContainer)
            ...
            coRequest.CookieContainer = cookieContainer
            ...
            Dim io As System.IO.Stream
            'Using f As New StreamWriter("C:\Users\Produzione.GR-RD-004\Desktop\grsrlrd-flashedlol-corretto\FlashedLOL\bin\Debug\boh.txt")
            '    f.WriteLine("strFields")
            '    f.Write(strFields)
            '    f.WriteLine("strFileHdr")
            '    f.Write(strFileHdr)


            '    f.WriteLine("aFilename")
            '    Dim allmem = File.ReadAllBytes(aFilename)
            '    For i = 0 To allmem.Length - 1
            '        f.Write(Chr(allmem(i)))
            '    Next
            ...
            '            f.Write(Chr(fileData(i)))
            '        Next
            '    End While
            '    readIn.Close()
            '    f.WriteLine("strFileTlr")
            '    f.Write(strFileTlr)
            'End Using
            io = getStream()
        ...
        Public Function getFileheader(ByVal aFilename As String) As String
            Return (Convert.ToString((Convert.ToString(ContentBoundary & Convert.ToString(vbCr & vbLf)) & CONTENT_DISP) + """file""; filename=""" + Path.GetFileName(aFilename) + """" & vbCr & vbLf + "Content-type: ") & FileContentType) + vbCr & vbLf & vbCr & vbLf
        End Function
        ''' <summary>
        ...
        Public Sub writeFile(ByVal io As System.IO.Stream, ByVal aFilename As String)
            Dim allmem = File.ReadAllBytes(aFilename)
            io.Write(allmem, 0, allmem.Length)
        End Sub
